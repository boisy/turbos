...
* Initialize the clock hardware
               ...
               sta       3,x                 enable DDRB
               coma                          invert A (%0000000 -> %11111111)
               sta       2,x                 set port B all outputs
               ldd       #$3435              IRQ needs to be left enabled
...
*
* For CoCo, called once every 16.667 milliseconds.
SvcIRQ         clra                          clear A         
               tfr       a,dp                set direct page to zero
               tst       PIA0Base+3          get 6821 control byte
               bmi       cocotimer@          branch if sync flag on, indicating interrupt source is the CoCo's timer
               jmp       [>D.SvcIRQ]         else service other possible IRQs
cocotimer@     tst       PIA0Base+2          clear the CoCo's timer interrupt
