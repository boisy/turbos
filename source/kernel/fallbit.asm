...
;;;         D = The number of the first bit to set.
...
*        X = The pointer to the bitmask table.
*
...
CalcBit        pshs      b                   preserve B
               ...
               rorb                          by 2
               lsra                          and divide D
               rorb                          again by 2, now D = D/8, or the byte offset to the bit
               leax      d,x                 get address of byte in bitmap where bit resides into X
               puls      b                   recover B to compute the bit
               lda       #$80                load A with hi bit set
               andb      #%0000111           mask out all but the lower 3 bits
...
loop@          anda      _mask@,s            AND with mask on stack
               ...
               asr       _mask@,s            else shift right the mask byte on the stack
               bcs       loop@               and continue if carry set
